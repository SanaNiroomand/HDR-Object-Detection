...
#
"""
cfg_hdr4rtt_rod_dedup.py -- identical to cfg_hdr4rtt_rod.py but trained on the
DEDUPLICATED split, so the headline table sits on the same footing as the
tone-mapping table.
...
class Exp(_base.Exp):
    def __init__(self):
        super().__init__()
        self.train_ann = "hdr4rtt_rod_dedup_train.json"
        self.val_ann = "hdr4rtt_rod_dedup_test.json"
        self.exp_name = "cfg_hdr4rtt_rod_dedup"
